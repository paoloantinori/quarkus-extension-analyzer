...
COMPLETED 2026-08-14 as the harvest facade: io.github.paoloantinori.qea.plugin.harvest.QuarkusExtensionMetadata (public API in core). Exposes config roots (+per-source provenance), deployment-artifact GAV, extension dependencies, contained classes, and deployment vocabulary from any extension jar, no augmentation needed. Validated on synthetic jars (3 unit tests) AND a real extension (quarkus-scheduler 3.33.3.1: configRoots=[quarkus.scheduler.], deploymentGav resolved, 7 extensionDeps, 33 classes, vocabulary=57 types, 0 errors). DECISION on repo extraction: deferred with evidence. The API is young, has no external consumers, and a separate release train would freeze it prematurely. The facade makes core genuinely reusable NOW; extract to a standalone project when real consumers exist. 96/96 reactor tests green.
<!-- SECTION:NOTES:END -->
